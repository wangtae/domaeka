...
def test_double_three_patterns():
    ...
    print(f"삼삼인가? {is_double_three(board1, 7, 7, 'B')}")
    ...
    print(f"삼삼인가? {is_double_three(board2, 6, 6, 'B')}")
    ...
    print(f"삼삼인가? {is_double_three(board3, 7, 7, 'B')}")
...
def test_specific_double_three_case():
    ...
    for x, y in candidates:
        result = is_double_three(board, x, y, 'B')
        print(f"좌표 ({x},{y}) → 금수: {result}")
    # 자동 검증: 모두 금수(True)여야 함
    for x, y in candidates:
        assert is_double_three(board, x, y, 'B'), f"{x},{y}는 삼삼 금수여야 함"
...
def test_compare_custom_vs_ruleengine_double_three():
    ...
    from games.omok.engine.rule_engine import is_double_three as engine_is_double_three
    size = 15
    # 다양한 보드 케이스
    test_cases = []
    # 1. 십자형 삼삼
    ...
    for idx, (board, x, y, color, expected) in enumerate(test_cases):
        custom = is_double_three(board, x, y, color)
        engine = engine_is_double_three(board, x, y, color)
        print(f"테스트 {idx+1}: ({x},{y}) → 커스텀={custom}, rule_engine={engine}, 기대={expected}")
        if custom != engine:
            print("[디버그] 판정 불일치! 내부 패턴/윈도우/인덱스 비교:")
            # 각 방향별로 내부 패턴 매칭 결과 출력
            directions = [(1, 0), (0, 1), (1, 1), (1, -1)]
            for dx, dy in directions:
                print(f"  방향 {(dx,dy)}:")
                # 커스텀 내부
...
def test_user_case_k5():
    ...
    # 금수 판정 좌표
    k5 = (10,4)
    custom = is_double_three(board, k5[0], k5[1], 'B')
    engine = engine_is_double_three(board, k5[0], k5[1], 'B')
