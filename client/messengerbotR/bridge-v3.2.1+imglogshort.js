 ...
 * 
 * @version 3.2.1+imglogshort
 * @author kkobot.com
 * @improvements 연결 안정성 개선, 모듈화, 멀티 서버 지원, 고급 미디어 전송, 이미지 로그 축약
 */
...
var Utils = (function() {
    ...
    // 🔴 이미지 로그 축약을 위한 헬퍼 함수
    function formatImageLog(message, isDetail) {
        // IMAGE_BASE64 패턴 감지
        if (message.indexOf('"IMAGE_BASE64:') !== -1 || message.indexOf('IMAGE_BASE64:') !== -1) {
            // Base64 데이터 추출 및 카운트
            var imageMatches = message.match(/IMAGE_BASE64:([^"]*)/);
            if (imageMatches && imageMatches[1]) {
                var base64Data = imageMatches[1];
                var imageCount = base64Data.split('|||').length;
                var totalSize = base64Data.length;
                
                // 축약된 로그 생성
                var shortLog = message.substring(0, message.indexOf('IMAGE_BASE64:')) + 
                              'IMAGE_BASE64:[' + imageCount + '개 이미지, 총 ' + 
                              Math.round(totalSize / 1024) + 'KB]';
                
                // 디테일 모드가 아니면 축약 로그 반환
                if (!isDetail) {
                    return shortLog;
                }
            }
        ...
        try {
            ...
            if (BOT_CONFIG.LOGGING.MESSAGE_TRANSFER) {
                if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT_DETAIL) {
                    // 🔴 이미지 로그 축약 처리
                    var logMessage = Utils.formatImageLog(jsonStr, true);
                    Log.i("[SEND] " + packet.event + " 메시지 전송 완료 - 전체내용: " + logMessage);
                } else if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT) {
                    // 🔴 이미지 로그 축약 처리
                    var logMessage = Utils.formatImageLog(jsonStr, false);
    ...
    function _handleServerResponse(rawMsg) {
        try {
            ...
            if (BOT_CONFIG.LOGGING.MESSAGE_TRANSFER) {
                if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT_DETAIL) {
                    // 🔴 이미지 로그 축약 처리
                    var logMessage = Utils.formatImageLog(rawMsg, true);
                    Log.i("[RECV] " + event + " 메시지 수신 - 전체내용: " + logMessage);
                } else if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT) {
                    // 🔴 이미지 로그 축약 처리
                    var logMessage = Utils.formatImageLog(rawMsg, false);
                    if (logMessage.length > 200) {
                        logMessage = logMessage.substring(0, 200) + "...";
                    }
