 ...
 * 
 * @version 3.2.2+smartlog
 * @author kkobot.com
 * @improvements 연결 안정성 개선, 모듈화, 멀티 서버 지원, 고급 미디어 전송, 스마트 로그 축약
 */
...
var BOT_CONFIG = {
    ...
    LOGGING: {
        ...
        LOG_TRUNCATE_LENGTH: 1000       // 🔴 로그 축약 기준 길이
    },
...
var Utils = (function() {
    ...
    // 🔴 스마트 로그 포맷팅 함수
    function formatLogPacket(packet, isDetail) {
        try {
            // packet 복사 (원본 수정 방지)
            var logPacket = JSON.parse(JSON.stringify(packet));
            
            // data.text 필드 체크 및 축약
            if (logPacket.data && logPacket.data.text && logPacket.data.text.length > BOT_CONFIG.LOGGING.LOG_TRUNCATE_LENGTH) {
                var text = logPacket.data.text;
                
                // IMAGE_BASE64 처리
                if (text.startsWith("IMAGE_BASE64:")) {
                    var base64Part = text.substring(13);
                    var imageCount = base64Part.split("|||").length;
                    var totalSize = Math.round(base64Part.length / 1024);
                    logPacket.data.text = "IMAGE_BASE64:[" + imageCount + "개 이미지, 총 " + totalSize + "KB]";
                } 
                // MEDIA_URL 처리
                else if (text.startsWith("MEDIA_URL:")) {
                    var urlPart = text.substring(10);
                    var urlCount = urlPart.split("|||").length;
                    logPacket.data.text = "MEDIA_URL:[" + urlCount + "개 미디어 URL]";
                }
                // 일반 긴 텍스트
                else {
                    logPacket.data.text = text.substring(0, 100) + "...[" + (text.length - 100) + "자 생략]";
                }
            }
            
            return JSON.stringify(logPacket);
        } catch (e) {
            // 에러 시 원본 반환
    ...
    return {
        ...
        formatLogPacket: formatLogPacket
    };
})();
        ...
        try {
            ...
            if (BOT_CONFIG.LOGGING.MESSAGE_TRANSFER) {
                if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT_DETAIL) {
                    // 🔴 스마트 로그 축약 처리
                    var logMessage = Utils.formatLogPacket(packet, true);
                    Log.i("[SEND] " + packet.event + " 메시지 전송 완료 - 전체내용: " + logMessage);
                } else if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT) {
                    // 🔴 스마트 로그 축약 처리
                    var logMessage = Utils.formatLogPacket(packet, false);
                    Log.i("[SEND] " + packet.event + " 메시지 전송 완료 - 내용: " + logMessage);
                } else {
        ...
        try {
            ...
            if (BOT_CONFIG.LOGGING.MESSAGE_TRANSFER) {
                if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT_DETAIL) {
                    // 🔴 스마트 로그 축약 처리
                    var logMessage = Utils.formatLogPacket(packet, true);
                    Log.i("[RECV] " + event + " 메시지 수신 - 전체내용: " + logMessage);
                } else if (BOT_CONFIG.LOGGING.MESSAGE_CONTENT) {
                    // 🔴 스마트 로그 축약 처리
                    var logMessage = Utils.formatLogPacket(packet, false);
                    Log.i("[RECV] " + event + " 메시지 수신 - 내용: " + logMessage);
                } else {
...
Log.i("[CONFIG] 로그 축약: " + BOT_CONFIG.LOGGING.LOG_TRUNCATE_LENGTH + "자 이상 메시지");
