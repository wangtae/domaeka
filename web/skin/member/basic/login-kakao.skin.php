<?php
if (!defined('_GNUBOARD_')) exit; // 개별 페이지 접근 불가

// add_stylesheet('css 구문', 출력순서); 숫자가 작을 수록 먼저 출력됨
add_stylesheet('<link rel="stylesheet" href="'.$member_skin_url.'/style.css">', 0);
?>

<style>
@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700&display=swap');

.kakao-login-wrapper {
    font-family: 'Noto Sans KR', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
    min-height: 100vh;
    background: white;
}

.kakao-login-nav {
    display: flex;
    align-items: center;
    justify-content: center;
    border-bottom: 1px solid #e5e7eb;
    background: white;
}

.kakao-login-header {
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 0 24px;
    height: 64px;
    width: 100%;
    max-width: 1024px;
}

.kakao-login-title {
    font-weight: bold;
    font-size: 18px;
}

.kakao-login-content {
    text-align: center;
    margin-top: 24px;
    margin-bottom: 32px;
    padding: 0 20px;
}

.kakao-login-subtitle {
    font-size: 18px;
    line-height: 1.5;
}

.kakao-login-main {
    display: block;
    margin-top: 8px;
    font-size: 24px;
    font-weight: 500;
    line-height: 1.4;
}

.kakao-login-button-wrapper {
    margin: 0 20px;
}

.kakao-login-links {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 20px;
    margin: 40px auto 0;
    padding: 0 20px;
    max-width: 400px;
}

.kakao-login-links a {
    color: black;
    text-decoration: underline;
    font-size: 14px;
    text-align: center;
}

/* 카카오 로그인 버튼 전체 감싸기 */
.kakao-login-button-wrapper {
    padding: 0 20px;
    margin: 0 auto;
    max-width: 600px;
}

.kakao-login-button-wrapper .sns-login {
    display: block !important;
    margin: 0 !important;
}

.kakao-login-button-wrapper .sns-login h3 {
    display: none !important;
}

.kakao-login-button-wrapper .sns-login .sns-wrap {
    display: block !important;
}

/* 카카오 로그인 버튼 스타일 */
.kakao-login-button-wrapper .sns-login a.sns-kakao {
    display: block !important;
    width: 100% !important;
    height: 60px !important;
    position: relative !important;
    overflow: hidden !important;
    border-radius: 8px !important;
    text-decoration: none !important;
    margin: 0 !important;
    padding: 0 !important;
}

/* 카카오 버튼 배경 이미지 */
.kakao-login-button-wrapper .sns-login a.sns-kakao::before {
    content: '' !important;
    position: absolute !important;
    top: 0 !important;
    left: 0 !important;
    right: 0 !important;
    bottom: 0 !important;
    background-image: url('data:image/png;base64,') !important;
    background-size: cover !important;
    background-position: center !important;
    background-repeat: no-repeat !important;
}

.kakao-login-button-wrapper .sns-login a.sns-kakao:hover::before {
    opacity: 0.95 !important;
}

/* 텍스트 숨기기 */
.kakao-login-button-wrapper .sns-login a.sns-kakao .ico,
.kakao-login-button-wrapper .sns-login a.sns-kakao .txt {
    display: none !important;
}

/* 다른 소셜 로그인 숨기기 */
.kakao-login-button-wrapper .sns-login a:not(.sns-kakao) {
    display: none !important;
}
</style>

<script>
// JavaScript 오류를 방지하기 위한 기본 함수 정의
if (typeof $ === 'undefined' && typeof jQuery !== 'undefined') {
    window.$ = jQuery;
}

// 소셜 로그인 팝업 처리 (팝업 사용 안함으로 설정되어 있으므로 페이지 이동)
function social_send_sns(provider, frm) {
    var url = frm.url.value || '';
    var link = '<?php echo G5_SOCIAL_LOGIN_URL; ?>/login.php?provider=' + provider;
    if (url) {
        link += '&url=' + encodeURIComponent(url);
    }
    window.location.href = link;
    return false;
}

// 페이지 로드 후 소셜 로그인 버튼 클릭 이벤트 처리
document.addEventListener('DOMContentLoaded', function() {
    // 카카오 로그인 버튼에 직접 클릭 이벤트 추가
    var kakaoLinks = document.querySelectorAll('.social_link_kakao');
    kakaoLinks.forEach(function(link) {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            var provider = 'Kakao';
            var url = '<?php echo isset($url) ? $url : ''; ?>';
            var link = '<?php echo G5_SOCIAL_LOGIN_URL; ?>/login.php?provider=' + provider;
            if (url) {
                link += '&url=' + encodeURIComponent(url);
            }
            window.location.href = link;
        });
    });
});
</script>

<div class="kakao-login-wrapper">
    <!-- Navigation -->
    <nav class="kakao-login-nav">
        <header class="kakao-login-header">
            <div class="kakao-login-title">우리공구 로그인</div>
        </header>
    </nav>
    
    <!-- Main Content -->
    <div class="kakao-login-content">
        <span class="kakao-login-subtitle">간편하게 로그인하고</span><br>
        <span class="kakao-login-main">
            우리동네 공동구매를<br> 
            경험해 보세요
        </span>
    </div>
    
    <!-- Kakao Login Button -->
    <div class="kakao-login-button-wrapper">
        <?php
        // 소셜로그인 사용시 소셜로그인 버튼
        if ($config['cf_social_login_use']) {
            // social_login_html이 비어있으면 직접 include
            if ($social_login_html) {
                echo $social_login_html;
            } else {
                // URL 인코딩
                $urlencode = urlencode($url);
                // 소셜 로그인 skin 직접 include
                @include_once(get_social_skin_path().'/social_login.skin.php');
            }
        }
        ?>
    </div>
    
    <!-- Additional Links -->
    <div class="kakao-login-links">
        <a href="<?php echo G5_BBS_URL ?>/login.php<?php echo $url ? '?url='.$url : ''; ?>">
            전화번호 로그인
        </a>
    </div>
</div>