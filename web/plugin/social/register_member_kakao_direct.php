...
if (!$user_profile) {
    ...
    print_r($_SESSION);
    echo "</pre>";
    die('카카오 로그인 정보가 없습니다. 위의 세션 데이터를 확인해주세요.');
}

if (!$provider_name) {
    die('Provider 정보가 없습니다.');
}
...
if (!$result) {
    die('회원가입 중 오류가 발생했습니다.');
}

// 소셜 프로필 정보 저장
$sql = " INSERT INTO {$g5['social_profile_table']} SET
            mb_id = '".sql_real_escape_string($mb_id)."',
            provider = '".sql_real_escape_string($provider_name)."',
            identifier = '".sql_real_escape_string($user_profile->identifier)."',
            profileurl = '".sql_real_escape_string($user_profile->profileURL)."',
            photourl = '".sql_real_escape_string($user_profile->photoURL)."',
            displayname = '".sql_real_escape_string($user_profile->displayName)."',
            description = '".sql_real_escape_string($user_profile->description)."' ";
